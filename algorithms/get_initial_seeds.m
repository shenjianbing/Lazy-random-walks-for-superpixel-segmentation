...
%get the initial seeds by place seed points in the image uniformly
%
if nargin < 4
    method = 'original';
end
...
%     mag = imfilter(mag, fspecial('average',5));
    ...
    [dx,dy] = local_min(mag,ceil(maxShift/2));
    new_x = dx(round(x(1,:)),round(y(:,1)));
    new_y = dy(round(x(1,:)),round(y(:,1)));
    x = new_x;
    y = new_y;
    off = 2;
    x = max(1+off,min(x,size(img,1)-off));
    y = max(1+off,min(y,size(img,2)-off));
