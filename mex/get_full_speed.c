  ...
  for (i = 1; i < iWidth - 1; i++)
  {
    ...
    { 
      ...
      double dCurvature = (dxx[i*iHeight+j]*dy_2 -
                           ...
                           dyy[i*iHeight+j]*dx_2) /((dx_2 + dy_2)*mag + eps);
      dCurvature = MAX(-1,MIN(dCurvature,1));
      doublet = MAX(0,(dx_norm*speed_x[i*iHeight+j] + dy_norm*speed_y[i*iHeight+j]));
	  s = grad_speed[i*iHeight+j]*(1-0.3*dCurvature) - doubletWeight*doublet;
